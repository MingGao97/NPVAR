...
#'
#'
#' @param x A n*p data matrix/dataframe w/o names
#' @param run.np Estimate conditional variance using kernel regression
#' @param run.mgcv Estimate conditional variance using GAM,
#'  If both are True, conduct both and output comparison for two methods
#'  Use GAM for final result
#' @param verbose Whether output detailed estimation process
#' @param layer.select Whether do layer selection
#' @param eta If layer.select is True, use this as fix eta for layer selection
...
#'  and a list of layers. If not, return a vector ancestors of ordering
NPVAR = function(x,
                 run.np = FALSE,
                 run.mgcv = TRUE,
                 verbose = FALSE,
...
est.condvars = function(x, descendants, node.index, ancestors, verbose, run.np, run.mgcv) {
  ...
  for(j in seq_along(descendants)){
    ...
    if(run.mgcv){
      ...
      fit.gam = predict(b1) # BAD NEWS BEARS
